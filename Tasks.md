...
- [x] **TDD:** PATH resolution (`which unity` / `Get-Command`), settings override, clear error if missing.
- [x] **TDD:** parse `unity --version --format json`; fail if CLI is below the documented minimum (current beta).
- [x] Rider setting: CLI path + default timeout (`UNITY_TEST_TIMEOUT`).
- [x] Support policy: Editor = current LTS only (`6000.0` / `2022.3` final); CLI = beta floor until a stable CLI exists.
...
- [x] GitHub Actions quality gate: Core `dotnet test` + `PluginXmlTest`.
- [ ] `./gradlew :check` in CI.
